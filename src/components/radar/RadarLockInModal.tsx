...
import type { SizingMode } from '../portfolio/PortfolioSettings';
import { getLocalConfig } from '../portfolio/PortfolioSettings';
import PortfolioSelector from '../portfolio/PortfolioSelector';
import type { PortfolioInfo } from '../portfolio/usePortfolios';

interface RadarPickInfo {
  id: string;
  ticker: string;
  name: string;
  price: number;
...
interface Props {
  ...
  onConfirm: (params: { spikeId: string; portfolioId: string; shares?: number; positionSize?: number; portfolioSize?: number; mode: SizingMode }) => Promise<void>;
  onCancel: () => void;
}

export default function RadarLockInModal({ pick, portfolios, activePortfolioId, onConfirm, onCancel }: Props) {
  ...
  if (mode === 'auto') {
    ...
    const kellyRaw = (winRate / (stopPct * 0.5)) - ((1 - winRate) / stopPct);
    const kelly = Math.min(Math.max(kellyRaw * 0.5, 0), maxPct);
    const posSize = config.portfolioSize * kelly;
    shares = Math.floor(posSize / pick.price);
    totalValue = shares * pick.price;
  } else if (mode === 'fixed') {
    const amount = Number(fixedInput) || 0;
    shares = Math.floor(amount / pick.price);
    totalValue = shares * pick.price;
  } else {
    ...
    if (inputType === 'shares') {
      ...
      totalValue = val;
      shares = Math.floor(val / pick.price);
      totalValue = shares * pick.price;
    }
  }

  const stopLossPrice = pick.price * 0.95;
      ...
      <div className="glass-card p-6 w-full max-w-lg mx-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="bg-spike-bg/50 rounded-xl p-4 mb-5 border border-spike-border/30">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-lg font-bold text-spike-text">{pick.ticker}</p>
              <p className="text-sm text-spike-text-dim">{pick.name}</p>
            </div>
            <p className="text-2xl font-bold mono">{formatCurrency(pick.price)}</p>
          </div>
          <div className="flex gap-4 mt-2">
            {pick.smartMoneyScore != null && (
              <span className="text-xs text-spike-cyan">Score: {pick.smartMoneyScore}</span>
            )}
            {pick.topCatalyst && (
              <span className="text-xs text-spike-text-dim">{pick.topCatalyst}</span>
            )}
          </div>
        </div>
        ...
        <div className="space-y-3 mb-5">
          ...
          <div className="flex justify-between text-sm">
            <span className="text-spike-text-dim">Stop-Loss (5%)</span>
